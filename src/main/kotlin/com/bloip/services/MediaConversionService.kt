...
class MediaConversionService {
    @Autowired private val applicationProperties: ApplicationProperties? = null
    @Autowired private val loggingService: LoggingService? = null
    ...
    fun init() {
        if (applicationProperties!!.enableRemoteServices == Constants.REMOTE_SERVICES_ON) {
            ...
            mediaConverterClient = buildAWSClientBuilder().withEndpointConfiguration(
                AwsClientBuilder.EndpointConfiguration(
                    getEndpointUrl(), applicationProperties.awsUploadRegion
                )
            ).build()
        } else {
    ...
    private fun getEndpointUrl() : String {
        val endPointUrl = buildAWSClientBuilder().build().describeEndpoints(DescribeEndpointsRequest()).endpoints[0].url
        loggingService!!.log("Endpoint: $endPointUrl")

        return endPointUrl
    }

    private fun buildAWSClientBuilder() : AWSMediaConvertAsyncClientBuilder {
        return AWSMediaConvertAsyncClient.asyncBuilder().
        withCredentials(
    ...
    fun convert(discussion: Discussion?, comment: Comment, discussionService: DiscussionService, commentService: CommentService) {
        executorService.execute {
            convertHelper(discussion, comment, discussionService, commentService)
        }
    }

    private fun convertHelper(discussion: Discussion?, comment: Comment, discussionService: DiscussionService, commentService: CommentService) {
        sendConversionRequest(
            keyName           = comment.fileName,
    ...
    private fun sendConversionRequest(keyName: String, discussion: Discussion?, comment: Comment, discussionService: DiscussionService, commentService: CommentService) {
        println("MediaConvertRole: ${applicationProperties!!.mediaConvertRole}")
        mediaConverterClient.createJobAsync(
            CreateJobRequest()
                ...
                .withPriority(0)
                .withHopDestinations()
                .withSettings(createJobSettings(keyName = keyName))
    ...
    private fun createJobSettings(keyName: String) : JobSettings {
        ...
        outputGroup.outputGroupSettings.fileGroupSettings.destination = "s3://${applicationProperties!!.awsUploadBucketName}/output/"
        ...
        outputGroup.outputGroupSettings.fileGroupSettings.destinationSettings.s3Settings.accessControl = S3DestinationAccessControl()
        outputGroup.outputGroupSettings.fileGroupSettings.destinationSettings.s3Settings.accessControl.cannedAcl = "PUBLIC_READ"
    ...
    class MediaConvertHandler : AsyncHandler<CreateJobRequest, CreateJobResult> {
        ...
        override fun onError(exception: Exception) {
            loggingService.error("Error converting file", exception)
        }

        override fun onSuccess(request: CreateJobRequest, result: CreateJobResult) {
            val status = result.job.status
            loggingService.log("MediaConvert Status: $status")

            if (discussion != null) {
                discussion.audioConversionInProgress = false
                discussion.active                    = true
                this.discussionService.save(discussion)
            }
